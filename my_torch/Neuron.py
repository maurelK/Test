...
class Neuron:
    def __init__(self, loss='categorical_crossentropy', learning_rate=0.01, l2_lambda=0.0, dropout_rates=None):
        ...
        self.cost_function, self.cost_derivative = define_cost_function(loss)
        self.loss_name = loss
    ...
    def backward(self, y_true, y_pred):
        # loss_grad = self.cost_derivative(y_true, y_pred)
        loss_grad = self.cost_derivative(y_true, y_pred)    
    ...
    #     # print(f"Training started: {n_samples} samples, batch_size={batch_size}, epochs={epochs}")
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, epochs=100, batch_size=32, verbose=True):
        ...
        for epoch in range(epochs):
            ...
            for batch in range(n_batches):
                ...
                if verbose:
                    print(f"  Batch {batch+1}/{n_batches} - Loss: {batch_loss:.4f}, Correct: {batch_correct}/{X_batch.shape[1]}")
                self.backward(y_batch, y_pred)
            ...
            #         print(f">>> Epoch {epoch+1} Summary: Train Loss: {avg_loss:.4f}, Acc: {train_accuracy:.2%} | "
