...
bool can_incant(tile_t *tile, int level, info_t *info)
{
    ...
    for (int i = 0; i < tile->player_count; i++) {
        int player_id = tile->player_ids[i];
        if (info->valid[player_id] && info->game.players[player_id].level == level)
            matching_players++;
    }
...
void consume_resources(tile_t *tile, int level)
{
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (tile->resources[i] < 0) tile->resources[i] = 0;
    }
}
...
void elevate_players(tile_t *tile, int level, info_t *info)
{
    ...
    for (int i = 0; i < tile->player_count && upgraded < required_players; i++) {
        int player_id = tile->player_ids[i];
        if (info->valid[player_id] && info->game.players[player_id].level == level) {
            info->game.players[player_id].level++;
            dprintf(info->data_socket[player_id], "Current level: %d\n", info->game.players[player_id].level);
            dprintf(1, "plv %d %d\n", player_id, info->game.players[player_id].level);
            upgraded++;
        }
    }
}
