#ifndef AES_CIPHER_HPP_
#define AES_CIPHER_HPP_

#include <string>
#include <vector>
#include <array>

class AesCipher {
public:
    AesCipher(const std::string& key);
    std::string cipher_block(const std::string& message);
    std::string decipher_block(const std::string& message);
    std::string cipher_stream(const std::string& message);
    std::string decipher_stream(const std::string& message);

private:
    std::vector<unsigned char> key_;
    static const int Nb = 4;  // Number of columns in state
    static const int Nk = 4;  // Number of 32-bit words in key
    static const int Nr = 10; // Number of rounds
    
    // AES tables
    const unsigned char sBox[256] = {
        
    };

    const unsigned char invSBox[256] = {
        
    };



    std::vector<unsigned char> convert_32bit_numbers_to_bytes(const std::string& numbers_hex);
    std::string convert_bytes_to_32bit_numbers(const std::vector<unsigned char>& bytes);
    std::vector<unsigned char> message_to_32bit_numbers(const std::string& message);
    std::string numbers_to_message(const std::vector<unsigned char>& bytes);

    // Helper functions
    void SubBytes(uint8_t state[4][4]);
    void InvSubBytes(uint8_t state[4][4]);
    void ShiftRows(uint8_t state[4][4]);
    void InvShiftRows(uint8_t state[4][4]);
    void MixColumns(uint8_t state[4][4]);
    void InvMixColumns(uint8_t state[4][4]);
    void AddRoundKey(uint8_t state[4][4], const uint32_t* roundKey);
    
    uint32_t SubWord(uint32_t word);
    uint32_t RotWord(uint32_t word);
    uint8_t gfMultiply(uint8_t a, uint8_t b);
    
    void KeyExpansion(const uint8_t key[16], uint32_t roundKey[44]);
    uint32_t Rcon(int round);

    // Utility functions
    std::vector<unsigned char> pad_data(const std::vector<unsigned char>& data, size_t block_size);
    std::vector<unsigned char> remove_padding(const std::vector<unsigned char>& data);
    void state_to_bytes(const uint8_t state[4][4], std::vector<unsigned char>& bytes);
    void bytes_to_state(const std::vector<unsigned char>& bytes, uint8_t state[4][4]);
};

#endif /* !AES_CIPHER_HPP_ */